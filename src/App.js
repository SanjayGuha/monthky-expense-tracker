...
function App() {
  ...
  // Define all functions after state initialization
  const updateAllExpenses = useCallback((newExpenses) => {
  ...
  const handleAddExpense = useCallback((expense) => {
    ...
    updateAllExpenses(newExpenses);
  }, [expenses, updateAllExpenses]);

  const handleAddFolder = useCallback((folder) => {
  ...
  const handleMoveToFolder = useCallback((expenseId, folderId) => {
    setExpenses(prevExpenses => {
      const updatedExpenses = prevExpenses.map(expense => {
        if (expense.id === expenseId) {
          return { ...expense, folderId };
        }
        return expense;
      });
